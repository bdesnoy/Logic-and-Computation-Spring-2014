...
There is an error because (or 1 nil) does not satisfy the input-contract
because (booleanp 1) returns nil.
...
; No, the function equal can take two "Any"s as input, while the function
; iff is similar but can only take booleans as input. Therefore, while
; giving iff two non-boolean inputs would lead to a contract violation,
; there are no inputs that can would cause a contract violation for equal
; but not for iff.
...
(defunc divides (x y)
  :input-contract (andand (natp x) (natp y) (not (equal y 0)))
  :output-contract (booleanp (divides x y))
  (integerp (/ x y)))

(check= (divides 10 5) t)
(check= (divides 10 4) nil)
(check= (divides 3 1) t)
...
(check= (endp ()) t)
(check= (endp (cons 5 ())) nil)
(check= (endp nil) t)
(check= (endp (cons nil nil)) nil)
...
(check= (has-three '(1 1)) nil)
(check= (has-three '(1 1 1)) t)
(check= (has-three '(1 1 1 1)) t)
...
(defunc n-th (n l)
  :input-contract (if (natp n) (listp l) nil)  
  :output-contract t
  (if (consp l)
    (if (< n 1)
      (first l)
...
; ('1) : ('1) is interpreted as a function call to '1 which is not an allowed 
;        function name and is not defined
; ((list)) : there is one extra set of parenthesis around the call to the list 
;            function
; (quote) : quote requires one arguement
; (if false 2 3) : false is not defined and ACL2 does not allow global variables
; ((+ 1 2)) : there is one extra set of parenthesis around the call to the + 
;             function, which is not allowed (mistaken for function call)
; list 1 2 : to call a function, parenthesis are needed around function and 
;            arguements
