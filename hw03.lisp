...
(defunc rem (x y)
  :input-contract (and (natp x) (natp y) (> y 0))
...
(defunc add-digits (x)
  ...
  (cond ((equal x 0) 0)
        (t (+ (rem x 10) (add-digits (/ (- x (rem x 10)) 10))))))

(check= (add-digits 0) 0)
(check= (add-digits 000) 0)
(check= (add-digits 123) 6)
(check= (add-digits 123000) 6)
(check= (add-digits 999) 27)
...
(defunc nat2list-aux (x)
  ...
  (cond ((equal x 0) nil)
        (t (cons (rem x 10) 
                 (nat2list-aux (/ (- x (rem x 10)) 10))))))

(check= (nat2list-aux 000) '())
(check= (nat2list-aux 123) '(3 2 1))
(check= (nat2list-aux 1023) '(3 2 0 1))
(check= (nat2list-aux 9090123) '(3 2 1 0 9 0 9))

(defunc nat2list (x)
  :input-contract (natp x)
  :output-contract (natlistp (nat2list x))
  (cond ((equal x 0) (cons 0 nil))
        (t (rev (nat2list-aux x)))))

(check= (nat2list 000) '(0))
(check= (nat2list 123) '(1 2 3))
(check= (nat2list 1023) '(1 0 2 3))
(check= (nat2list 9090123) '(9 0 9 0 1 2 3))
...
(defunc has-dups (l)
  ...
  (cond ((endp l) nil)
        (t (or (in (first l) (rest l))
               (has-dups (rest l))))))

(check= (has-dups '()) nil)
(check= (has-dups '(1))   nil)
(check= (has-dups '(1 1)) t)
(check= (has-dups '(1 2 3 2)) t)
(check= (has-dups '(1 2 3 2 3)) t)
...
(check= (how-many 1 '(1 2 1 2)) 2)
(check= (how-many 'a '(a b c d)) 1)
(check= (how-many t '(t nil nil t t)) 3)
...
(defunc scale-natlist (s l)
  ...
  (cond ((endp l) nil)
        (t (cons (* (first l) s) (scale-natlist s (rest l))))))

(check= (scale-natlist 3 '( 1 2 3)) '( 3 6 9))
(check= (scale-natlist 1 '(1 2 3 4)) '(1 2 3 4))
(check= (scale-natlist 2 '(1 1 1 1)) '(2 2 2 2))
...
+---+---+---+--------+--------+----------------------+--------+----------------------------------+
  ...
  (p => r) /\ (q => r) /\ (~p => ~r)
+---+---+---+--------+--------+----------------------+----+----+------------+------------------------------------+
| p | q | r | p => r | q => r | (p => r) /\ (q => r) | ~p | ~r | (~p => ~r) | (p => r) /\ (q => r) /\ (~p => ~r) |
+---+---+---+--------+--------+----------------------+----+----+------------+------------------------------------+
| F | F | F | T      | T      | T                    | T  | T  | T          | T                                  |
| F | F | T | T      | T      | T                    | T  | F  | F          | F                                  |
| F | T | F | T      | F      | F                    | T  | T  | T          | F                                  |
...
(1) When a is T and b is T, then (a /\ b) is true. Therefore, in order to
    make (a /\ b) => (a * b) true,  (a * b) must also be true.
(2) For (a * b) to not be equilivent to (a /\ b), (a \/ b) a => b, a <> b,
    , or (a = b), then the value of (a * b) when a is F and b is T must be
    F. If the value of (a * b) were T in this case, (a * b) would be the
    same as (a \/ b).

(b) Is this connective really new? How can you simplify a * b ?

(a * b) can be simplified as a.
...
Yes, we need this connetive if we want to express ~(~a /\ b) with a single
connective. There is no single connective that we have discussed in class
...
(1) For h to be reflexive, h(T, T) and h(F, F) must be T.
(2) Because h is symmetric, h(T, F) must be the same as h(F, T). Because
    at least one input must make h false, h(T, F) and h(F, T) must be F.
...
~  : not T-reproducing because ~ is unary and ~x could produce both T and F
     depending on the value of x

\/ : not T-reproducing because if a and b are both false (a \/ b) would not
     be equivalent to T
